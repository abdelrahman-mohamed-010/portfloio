  ...
  return (
    <nav className="sm:px-16 px-6 w-full flex items-center py-5 fixed z-20 top-0  ">
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
          ...
          {navLinks.map((nav) => (
            <a
              ...
              className={classnames(
                "text-secondary hover:text-white text-[18px] font-medium cursor-pointer duration-300",
                { "text-white": active === nav.id },
                `animate__fadeInRight animate__animated animate__delay-1s`
              )}
            >
              {nav.title}
            </a>
        ...
        <div
          className={classnames(
            "flex p-6 black-gradient absolute top-20 right-2 mx-4 my-2 min-w-[140px] rounded-xl",
            { "hidden ": !toggle }
          )}
        >
